...
记时间常数 $\tau_{m}=\frac{C_{m}}{g_{m}}$ ，将方程两侧同除以 $g_{m}$ ，可得到如下方程：

$$\frac{I_{m}}{g_{m}}=\tau_{m}\frac{dV_{m}}{dt}+(V_{m}-v_{rest})$$

又由 $g_{m}=\frac{1}{R_{m}}$ ，移项后可得到一阶微分方程：

$$\tau_{m}\frac{dV_{m}}{dt}=-(V_{m}-v_{rest})+I_{m}R_{m}$$
...
$$V_{m}(t)-V_{m}(t-1)=\frac{1}{\tau_{m}}[-[V_{m}(t-1)-v_{rest}]+R_{m}I_{m}(t)]$$
...
$$\tau_{m}\frac{dV_{m}}{dt}=a_{0}(V_{m}-V_{rest})(V_{m}-V_{c})+I_{m}R_{m}$$
...
$$\tau_{m}\frac{dV_{m}}{dt}=-(V_{m}-V_{rest})+\Delta_{T}e^{\frac{V_{m}-v_{T}}{\Delta_{T}}}+I_{m}R_{m}$$

其加入了一个微小偏置 $\Delta_{T}e^{\frac{V_{m}-v_{T}}{\Delta_{T}}}$ ，用于修正因离子通透性随电位变化导致微小电导差异的影响。

### Adaptive Exponential Integrate-and-Fire （AdEx）模型

ExpIF模型的一阶反应微分方程为：

$$\tau_{m}\frac{dV_{m}}{dt}=-(V_{m}-V_{rest})+\Delta_{T}e^{\frac{V_{m}-v_{T}}{\Delta_{T}}}-w_{m}R_{m}+I_{m}R_{m}$$

$$\tau_{w}\frac{dw_{m}}{dt}=a(V_{m}-V_{rest})-w_{m}+b\tau_{w}\sum_{t^{(f)}}{\delta(t-t^{(f)})}$$
